...
partial class Program
{
  private static void FilterAndSort()
  {
    ...
    IQueryable<Product> filteredProducts = allProducts.ProcessSequence().Where(product => product.UnitPrice < 10M);

    IOrderedQueryable<Product> sortedAndFilteredProducts =
      filteredProducts.OrderByDescending(product => product.UnitPrice < 10M);
    ...
    {
      WriteLine("{0}: {1} cost less than {2:$#,##0.00}",
  ...
  private static void AggregateProducts()
  {
    ...
    // Try to get an efficient count from DF Core DbSet<T>
  ...
  private static void OutputTableOfProducts(Product[] products, int currentPage, int totalPages)
  {
    ...
    WriteLine("{0} Page of {1} of {2} {3}", lineHalf, currentPage + 1, totalPages + 1, lineHalf);
  }
  ...
  private static void PagingProducts()
  {
    ...
    while (true)
    {
      ...
      Write("Press <- to page back, press -> page forward, any key to exit.");
